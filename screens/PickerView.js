...
const PickerView = ({navigation,route}) => {
    ...
    useEffect(() => {
        const DATA=route.params.countrylist
        setfilterData(DATA)
        setmasterData(DATA)
                ...
                onPress={()=>
                    {
                        ...
                        navigation.navigate(`${navView}`,{countryName:item.countryName,countryId:item.countryId,countryCode:item.countryCode});
}
                    }
                />
                <Divider />
